...
const ENTRIES = [
  {
    ...
    decisions: ['之前一整天試的做法（REST imageTransform 重建裁切、CSS mix-blend 重現、alpha 剪影 IoU 匹配、回推透明度、逐節點 app 匯出、bg_texture 群組匯出）全部作廢。教訓：含混合模式的背景，隱藏其他圖層後匯整個 frame，不要匯群組或節點。', 'REST 讀到的 imageTransform 與四個 frame 相同，但照字面重建的裁切與算圖不符，且 4:3 frame 用的是另一張 lobby_bg 9-slice——四個比例的背景不能共用一張圖，之後要分別匯。'],
    ...
    files: ['assets-raw/lobby/bg_curtain.png', 'public/assets/lobby/bg_curtain.webp', 'src/views/lobby/LobbyBackdrop.tsx', 'src/views/lobby/lobbyAssets.ts', 'src/views/lobby/lobbyAssets.test.ts'],
  },
  {
    date: '2026-09-14',
