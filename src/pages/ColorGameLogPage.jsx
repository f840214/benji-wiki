...
const ENTRIES = [
  {
    ...
    decisions: ['jackpot 底條：稿是「JACKPOT + 金額」一組置中、金額變長時 JACKPOT 往左讓。原本 JACKPOT 固定 83.5 寬 + 金額 flex-1 → 改容器 justify-center，標籤自然寬不縮、金額 flex 0 1 auto 可縮（吃 LobbyStripText 的等比縮字），gap 8。幣別符號字型稿是 Baloo Bhaijaan 400（不是 Baloo 2 500）：main.tsx 載 @fontsource/baloo-bhaijaan-2/400、@theme 加 --font-bhaijaan、SYMBOL_CLASS 改 font-bhaijaan font-normal；document.fonts 確認 latin-ext 子集（含 ₱ U+20B1）loaded。', 'is-selected 綁 isFavorite：已收藏的桌在列表初次渲染時也會播一次回彈 + 光圈（cg-client 的 in 只在點擊時播）；先照使用者的版本，看過再決定要不要加 is-fresh 之類的閘。'],
    ...
    files: ['src/views/lobby/LobbyStripText.tsx', 'src/views/lobby/LobbyTableCard.tsx', 'src/index.css', 'src/main.tsx'],
  },
  {
    date: '2026-09-11',
