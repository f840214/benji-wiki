...
const ENTRIES = [
  {
    ...
    decisions: [
      ...
      '對稿改用「掃顏色邊界」:兩張 2× 圖同一欄由上而下列出顏色類別變化列、逐列相減。發現 Figma 匯出圖的卡頂在 y=1(陰影外溢只有 1px,不是 4),之前的疊圖整張偏 1.5 CSS px(結論是相對比較,不受影響)。校正後各邊界與稿差 ≤1 個 2× 像素(0.5 CSS)。試過把頂列以外元素下移 0.5 補償,結果只是把取整誤差搬到別的邊(有的邊變 +1、有的不動),所以撤回:座標一律用稿值,≤1 個 2× 像素視為對上;方法寫進 design-spec。',
